...
impl Builder for TosBuilder {
    ...
    fn build(self) -> Result<impl Access> {
        ...
        let endpoint_domain = if endpoint.starts_with("http://") {
            &endpoint[7..]
...
impl Access for TosBackend {
    ...
    async fn list(&self, path: &str, args: OpList) -> Result<(RpList, Self::Lister)> {
        let l = if args.versions() || args.deleted() {
            let lister = TosObjectVersionsLister::new(self.core.clone(), path, args);
            TwoWays::Two(oio::PageLister::new(lister))
        } else {
            let lister = TosLister::new(self.core.clone(), path, args);
            TwoWays::One(oio::PageLister::new(lister))
        };
        Ok((RpList::default(), l))
    }

    async fn copy(&self, from: &str, to: &str, _args: OpCopy) -> Result<RpCopy> {
        let resp = self.core.tos_copy_object(from, to).await?;
